...
lemma partialFourierSumL2_norm {T : ℝ} [hT : Fact (0 < T)] [h2 : Fact (1 ≤ (2 : ENNReal))] {f : ↥(Lp ℂ 2 haarAddCircle)} {N : ℕ} :
  ...
  calc ‖partialFourierSumLp 2 N f‖ ^ 2
    ...
    _ = ∑ n ∈ Finset.Icc (-Int.ofNat N) N, ‖fourierCoeff f n‖ ^ (2 : ℝ) := by
      rw [← coe_fourierBasis]
      simp only [LinearIsometryEquiv.apply_symm_apply, lp.coeFn_smul, Pi.smul_apply, ← lp.single_smul, Int.ofNat_eq_coe, coe_fourierBasis, Real.rpow_two]
      -- simp only [Int.ofNat_eq_coe, coe_fourierBasis, Real.rpow_two]
