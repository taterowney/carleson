...
lemma Dirichlet_Hilbert_diff {N : ℕ} {x : ℝ} (hx : x ∈ Set.Icc (-π) π) :
  ...
  calc ‖  (min |x| 1) * (exp (I * N * x) / (1 - exp (-I * x)))
    ...
    _ ≤ |x| * (1 / ‖1 - exp (I * x)‖) + |x| * (1 / ‖1 - exp (I * x)‖) := by
      ...
      rw [_root_.abs_of_nonneg (by sorry)]
      gcongr
      · sorry
      · sorry
...
lemma intervalIntegrable_mul_dirichletKernel'_max {x : ℝ} (hx : x ∈ Set.Icc 0 (2 * π)) {f : ℝ → ℂ}
  ...
  use 1
  intro y
  simp only [id_eq, Function.comp_apply, abs_ofReal]
  rw [_root_.abs_of_nonneg (sorry)]
  simp
  sorry
    ...
    _ ≤ ⨆ (n : ℤ) (r : ℝ) (_ : 0 < r) (_ : r < 1), (  ↑‖∫ y in {y | dist x y ∈ Set.Ioo r 1}, g y * K x y * exp (I * n * y)‖₊
      ...
      calc ‖∫ y in {y | dist x y ∈ Set.Ioo r 1}, g y * (exp (I * (-n * x)) * K x y * exp (I * n * y) + conj (exp (I * (-n * x)) * K x y * exp (I * n * y)))‖₊
        ...
        _ =   ‖∫ y in {y | dist x y ∈ Set.Ioo r 1}, g y * K x y * exp (I * n * y)‖₊
          ...
          norm_cast
          congr 1 <;>
          · rw [integral_mul_left, norm_mul, mul_comm I, abs_exp_ofReal_mul_I, one_mul]
