...
lemma kernel_bound {s : ℤ} {x y : X} :
  ...
  apply le_trans <| calc
    ...
    _ ≤ ‖K x y‖ * 1               := by gcongr; rw [abs_ofReal]; exact abs_ψ_le_one D _
...
lemma K_eq_K_of_dist_eq_zero {x y y' : X} (hyy' : dist y y' = 0) :
    K x y = K x y' := by
  suffices ‖K x y - K x y'‖ = 0 by sorry
...
lemma norm_Ks_le {s : ℤ} {x y : X} :
  ...
  · rwa [hK, norm_zero]
  rw [Ks, norm_mul, abs_ofReal, ← mul_one (_ / _)]
  gcongr
...
private lemma norm_Ks_sub_Ks_le₀ {s : ℤ} {x y y' : X} (hK : Ks s x y ≠ 0)
  ...
  norm_cast
  simp_rw [D2_1_3, norm_mul, abs_ofReal]
